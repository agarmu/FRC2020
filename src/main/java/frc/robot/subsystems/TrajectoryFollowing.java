...
public class TrajectoryFollowing extends SubsystemBase {
  ...
  public TrajectoryFollowing() {
    ...
    odometry =
        new DifferentialDriveOdometry(
            getHeading() /*, pose*/); // try with and without pose as argument
    ...
    // TrajectoryConfig trajectoryConfig = new TrajectoryConfig(Constants.maxVelocity,
    // 3).setKinematics(kinematics);

    // traj = TrajectoryGenerator.generateTrajectory(new Pose2d(0, 0, new Rotation2d(0)),
    // List.of(new Translation2d(1, 1), new Translation2d(1, 2)), new Pose2d(3, 0, new
    // Rotation2d(0)), trajectoryConfig);

    // RamseteCommand ramseteCommand = new RamseteCommand(traj, pose, new RamseteController(2, 0.7),
    // new SimpleMotorFeedforward(Constants.kVolts, Constants.kVSPM, Constants.kVSSPM), kinematics,
    // getSpeeds(), new PIDController(Constants.kPDriveVelocity, 0, 0), new
    // PIDController(Constants.kPDriveVelocity, 0, 0), thisTraj::tankDriveVolts, thisTraj::);
  }
  ...
  public DifferentialDriveWheelSpeeds getSpeeds() {
    return new DifferentialDriveWheelSpeeds(
        driveMasterLeft.getSelectedSensorVelocity() / (50 / 6) * 2 * Math.PI / 60,
        driveMasterRight.getSelectedSensorVelocity() / (50 / 6) * 2 * Math.PI / 60);
  }
  ...
  public void periodic() {
    pose =
        odometry.update(
            getHeading(),
            driveMasterLeft.getSelectedSensorPosition(),
            driveMasterRight.getSelectedSensorPosition());
  }
  ...
  public double getAverageEcoderDistance() {
    return ((driveMasterLeft.getSelectedSensorPosition() * .319 / 100)
            + (driveMasterRight.getSelectedSensorPosition() * .319 / 100))
        / 2;
  }
